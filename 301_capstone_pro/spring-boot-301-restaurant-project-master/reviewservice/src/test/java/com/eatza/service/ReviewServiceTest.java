...
class ReviewServiceTest {
	...
	public void deleteReviewByIdTest1() {
		...
		assertEquals("Deleted Successfully", reviewServiceImpl.deleteReviewById(review.getReviewId()));

	}

	@Test
	public void updateReviiewIfPresent()  {

		Long reviewId = 1L;
		Review review = getReview();
		Optional<Review> checkReview = Optional.of(review);
		when(reviewRepository.findById(reviewId)).thenReturn(checkReview);
		assertEquals("Success", reviewServiceImpl.updateReview(review));
